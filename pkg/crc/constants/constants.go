...
var ocURLForOs = map[string]string{
	"darwin":  fmt.Sprintf("%s/%s/%s-%s.%s", version.GetDefaultOcURLBase(), "openshift-client-mac", version.GetOcVersion(), "tar.gz"),
	"linux":   fmt.Sprintf("%s/%s/%s-%s.%s", version.GetDefaultOcURLBase(), "openshift-client-linux", version.GetOcVersion(), "tar.gz"),
	"windows": fmt.Sprintf("%s/%s/%s-%s.%s", version.GetDefaultOcURLBase(), "openshift-client-windows", version.GetOcVersion(), "zip"),
}
